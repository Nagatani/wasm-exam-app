...
- 提出モデルの刷新（2026-09-10）完了 — 設問ごとは「下書き保存」、試験全体で1回「最終提出」して採点。受験可能回数を講師が設定（既定1回 / 無制限可）、成績は最後に提出した回。時間切れは自動提出。マイグレーション `20260910120000_add_attempt_lifecycle` の適用が必要
- フェーズ6の残り（TLE/MLEの厳密化、Cの実行時間制限、エラーフィードバック改善ほか）未着手 — スコープと今後の展望は [`docs/roadmap.md`](./docs/roadmap.md)
