...
- **オプトイン式なので既定では誰もダウンロードしません**。AI作問サポート（教師専用）とAIヒント（生徒向け、演習モードの問題のみ）は別々のオプトインで、ユーザーメニュー👤から有効化した人のブラウザにだけモデルが入ります。学内回線への影響を抑えたい場合は、これらの機能を使う予定がなければ生徒・教師に有効化しないよう案内してください。
...
- モデルはオプトインを切っただけでは削除されません（再度有効化したときすぐ使えるように、キャッシュは残ります）。完全に消したい場合はユーザーメニューの「ダウンロード済みモデルを削除してオフにする」を使ってください（教師・生徒どちらの設定パネルから削除しても、同じブラウザ内の両機能に影響します — 同一モデルのキャッシュを共有しているためです）。
