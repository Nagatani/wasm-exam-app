...
両機能とも同じ `@mlc-ai/web-llm` モデル（`Qwen2.5-Coder-3B-Instruct-q4f16_1-MLC`、**約2.5GB**）をブラウザにダウンロードします。上記のclang/Pyodideより一桁大きいため、一斉アクセス対策の重要性はより高くなります。

- **オプトイン式なので既定では誰もダウンロードしません**。AI作問サポート（教師専用）とAIヒント（生徒向け、演習モードの問題のみ）は別々のオプトインで、☰メニュー →「設定（AI機能など）」から有効化した人のブラウザにだけモデルが入ります。学内回線への影響を抑えたい場合は、これらの機能を使う予定がなければ生徒・教師に有効化しないよう案内してください。
- **配信元はアプリ側で固定できません**（Wasmerレジストリ経由のclangと同様、`@mlc-ai/web-llm`が内部で参照するCDNから取得）。学内からその配信元に到達できない環境では、両機能とも初回ダウンロードに失敗します。
- クラス全体でAIヒントを使わせる授業がある場合は、事前に該当ブラウザで一度オプトイン・ダウンロードを済ませておく（clangの「演習室PCの事前ウォーム」と同じ考え方）ことを推奨します。
- モデルはオプトインを切っただけでは削除されません（再度有効化したときすぐ使えるように、キャッシュは残ります）。完全に消したい場合は☰メニュー →「設定（AI機能など）」の「ダウンロード済みモデルを削除してオフにする」を使ってください（教師・生徒どちらの設定パネルから削除しても、同じブラウザ内の両機能に影響します — 同一モデルのキャッシュを共有しているためです）。

## 生徒アカウントの一括作成と初期パスワード

講師は☰メニューの「クラス管理」から `学籍番号,氏名` の名簿で生徒アカウントを一括作成できます（`POST /api/students/bulk`）。
...
- **ログアウトだけさせたい**（共用PCでのログアウト忘れ、試験端末からの退出など）ときは、受講者一覧の「ログアウトさせる」または☰ →「管理者メニュー」の「強制ログアウト」を使います（`POST /api/students/force-logout`）。その生徒の有効なセッションがすべて失効しますが、パスワードは変わりません。教員アカウントは対象外です。
...
- **期限切れ・失効したログインセッションの削除**：サーバー起動時と、その後 `SESSION_CLEANUP_INTERVAL_HOURS`（既定6）時間ごとに、期限切れ（ログインから12時間経過）またはログアウト・強制ログアウト・パスワード再発行で失効したセッションを `sessions` テーブルから自動削除します（`0` で無効）。すでにログインに使えない行だけを消すので、利用者への影響はありません。
