...
Being built in explicit phases (see project memory / prior conversation for the full roadmap): Phase 1 (auth + roles) and Phase 2 (teacher exam/task/test-case authoring) are done; the in-browser C/Java judge, student exam-taking UI, and the teacher grade dashboard + CSV export come in later phases. Don't jump ahead to a later phase's code without the user's sign-off.
